  ...
  return (
    <nav 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-white/80 dark:bg-martial-black/80 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-6'
      }`}
    >
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            ...
            >
              <a 
                href={link.href}
                className={`text-sm font-semibold uppercase tracking-wider dark:text-gray-300 hover:text-${beltColor} dark:hover:text-${beltColor} transition-colors flex items-center gap-1`}
              >
                {link.name}
                {link.submenu && (
              ...
              {link.submenu && (
                <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-4 transition-all duration-300 ${isProgrammesOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-2 pointer-events-none'}`}>
                  <div className="w-60 bg-white dark:bg-martial-black rounded-2xl shadow-xl border border-gray-100 dark:border-white/10 p-2">
                    <div className="grid grid-cols-1 gap-1">
                      {link.submenu.map((sub) => (
                        <a 
                          key={sub.name}
                          href={sub.href}
                          className={`px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 hover:bg-martial-gray dark:hover:bg-white/5 hover:text-${beltColor} dark:hover:text-${beltColor} transition-all`}
                        >
                          {sub.name}
                        </a>
                      ))}
          ...
          <a 
            ...
            className={`bg-martial-black dark:bg-white text-white dark:text-martial-black px-6 py-2.5 rounded-full text-sm font-bold uppercase tracking-wide hover:bg-${beltColor} dark:hover:bg-${beltColor} dark:hover:text-white transition-all ml-4`}
          >
            Espace Élève
          </a>
        </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-white dark:bg-martial-black border-t border-gray-100 dark:border-white/10 overflow-hidden"
          >
            <div className="container-custom py-8 flex flex-col gap-6">
              {navLinks.map((link) => (
                <div key={link.name} className="flex flex-col gap-4">
                  <div className="flex items-center justify-between text-martial-black dark:text-white">
                    <a 
                      href={link.href}
